...
arr_histogram(PyObject *self, PyObject *args)
{
	...
	{
	    Py_Try(ans =
		   (PyArrayObject *) PyArray_SimpleNew (1, &ans_size, PyArray_INT));
	    ians = (int *) A_DATA (ans) ;
	    for (i = 0 ; i < len ; i++)
		ians [numbers [i]] += 1 ;
	    Py_DECREF(lst);
    ...
    else
	{
		...
		return NULL;}
	    Py_Try(ans =
		   (PyArrayObject *) PyArray_SimpleNew (1, &ans_size, PyArray_DOUBLE));
...
arr_index_sort(PyObject *self, PyObject *args)
{
    ...
    len = A_SIZE(alist);
    Py_Try(ilist = (PyArrayObject *) PyArray_SimpleNew (1, &len, PyArray_INT));
...
arr_digitize(PyObject *self, PyObject *args)
{
	...
	if (x_is_scalar)
	    ...
	    {
		aret = (PyArrayObject *) PyArray_SimpleNew (1, &lx, PyArray_LONG) ;
...
arr_reverse(PyObject *self, PyObject *args)
{
    ...
    Py_Try(ares = (PyArrayObject *) PyArray_SimpleNew (2, dims, PyArray_DOUBLE));
...
arr_span(PyObject *self, PyObject *args)
{
    ...
    dims [1] = num;
    dims [0] = d2;
    Py_Try(arow = (PyArrayObject *) PyArray_SimpleNew (1, &num, PyArray_DOUBLE));
	...
	{
	    Py_Try
		(ares = (PyArrayObject *) PyArray_SimpleNew (2, dims, PyArray_DOUBLE));
	    dres = (double *) A_DATA (ares) ;
...
{ /* construct3 (mask, itype) computes how the cut
    ...
    Py_Try(permutea =
	   (PyArrayObject *) PyArray_SimpleNew (2, permute_dims, PyArray_INT));
