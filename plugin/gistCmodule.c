	...
	static ArrayObject *allocateArray(int size, char tc, int nlist) {
		...
		if (size != 0) {
			...
			case 'd':
				if (!
				    (res->data =
				     (void *)calloc(size, sizeof(double)))) {
					free(res);
					return (ArrayObject *) PyErr_NoMemory();
				}
				break;
...
/*************************************************************************/
	...
	static int addToArrayList(PyObject * obj) {
		if (obj == (PyObject *) NULL
		    || array_list_length > ARRAY_LIST_SIZE)
			return 0;
...
/* Routines in the Gist user interface */
	...
	static PyObject *bytscl(PyObject * self, PyObject * args,
				PyObject * kd);
	static PyObject *contour(PyObject * self, PyObject * args,
				 PyObject * kd);
	static PyObject *current_window(PyObject * self, PyObject * args);
	static PyObject *pyg_fma(PyObject * self, PyObject * args);
	static PyObject *gridxy(PyObject * self, PyObject * args,
				PyObject * kd);
	static PyObject *hcp(PyObject * self, PyObject * args);
	static PyObject *hcp_file(PyObject * self, PyObject * args,
				  PyObject * kd);
	...
	static PyObject *limits(PyObject * self, PyObject * args,
				PyObject * kd);
	...
	static PyObject *pldefault(PyObject * self, PyObject * args,
				   PyObject * kd);
	static PyObject *pldj(PyObject * self, PyObject * args, PyObject * kd);
	static PyObject *pledit(PyObject * self, PyObject * args,
				PyObject * kd);
	...
	static PyObject *plmesh(PyObject * self, PyObject * args,
				PyObject * kd);
...
/* Utility routines */
	...
	static long unpack_color_tuple(PyObject * ob,
				       unsigned long color_triple[3]);
	static long unpack_limit_tuple(PyObject * ob, double limits[],
				       int *flags);
...
/* static int lenPrintBuf = 79; */
	...
	static PyObject *_slice2_precision;;
...
/*#######################################################################*/
	...
	static ArrayObject *logical_and(ArrayObject * a, ArrayObject * b,
		...
		for (; tar < (Uchar *) (res->data) + res->size;
		     src1++, src2++, tar++)
			*tar = *src1 && *src2;
		if (freea >= 0)
			freeArray(a, freea);
...
/************************************************************************/
	...
	static long _slice2_part(ArrayObject * xyzc, ArrayObject * keep,
		...
		int *nextd = 0,
		    /* * nvertcd=0, */
		    *prevd = 0,
		    *list0d = 0, *list1d = 0, *listd = 0, *xoldd = 0;
		/*####### */ int *ndxsd;
		double *xyz0d = 0,
		    *valcd = (double *)NULL,
		    *valc0d = (double *)NULL,
		    ...
		    *dpd = 0,
		    *xyzcd = 0,
		    *xyz1d = 0, *xyzc_newd = 0, *valc_newd = (double *)NULL;
		Uchar *valcc = (Uchar *) NULL,
		    *valc0c = (Uchar *) NULL,
		    *valc1c = (Uchar *) NULL, *valc_newc = (Uchar *) NULL;
		...
		*nvertc = (ArrayObject *) NULL;
		...
		if (nlist0 != 0) {
			...
			for (j = 0, i = 0; i < keep->size; i++)
				if (!keepd[i] && keepd[nextd[i]])
					list0d[j++] = i;
			TRY(xyz0 =
			    allocateArray(3 * nlist0, 'd', 1), (long)NULL);
			xyz0d = (double *)(xyz0->data);
			if (valc != (ArrayObject *) NULL) {
				TRY(valc0 =
				    allocateArray(nlist0, atype, 1),
				    (long)NULL);
				if (atype == 'd')
		...
		if (nlist1 != 0) {
			...
			for (j = 0, i = 0; i < keep->size; i++)
				if (!keepd[i] && keepd[prevd[i]])
					list1d[j++] = i;
			TRY(xyz1 =
			    allocateArray(3 * nlist1, 'd', 1), (long)NULL);
			xyz1d = (double *)(xyz1->data);
			if (valc != (ArrayObject *) NULL) {
				TRY(valc1 =
				    allocateArray(nlist1, atype, 1),
				    (long)NULL);
				if (atype == 'd')
		...
		TRY(list =
		    allocateArray(mask->size, 'i', 1), (long)PyErr_NoMemory());
		listd = (int *)(list->data);
		...
		TRY(xold =
		    allocateArray(listd[list->size - 1], 'i', 1), (long)NULL);
		xoldd = (int *)(xold->data);
		...
		TRY(*xyzc_new =
		    allocateArray(3 * xold->size, 'd', 0), (long)NULL);
		...
		for (i = 0; i < nlist0; i++) {
			...
			if (valc != (ArrayObject *) NULL) {
				if (atype == 'd')
					valc_newd[listd[list0d[i]] - 1] =
					    valc0d[i];
				else if (atype == 'b')
					valc_newc[listd[list0d[i]] - 1] =
					    valc0c[i];
			}
		}
		freeArray(list0, 1);
...
/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
	...
	static char *CheckDefaultWindow(void) {
		...
		if (curPlotter < 0) {
			...
			ghDevices[0].display =
			    GpFXEngine(windowNames[0], 0, defaultDPI,
				       (char *)0);
...
/* Called from bytscl, pli, plf, and plfp */
	static int GrabByteScale(PyObject * kwt[], char *keywrds[],
		...
		if (!minGiven || !maxGiven) {
			double zmn, zmx;
			GetZCrange(&zmn, &zmx, z, reg, region, iMax, jMax,
				   zCompressed);
			if (!minGiven)
				zmin = zmn;
			if (!maxGiven)
...
#define N_KEYWORDS 2
	...
	static PyObject *contour(PyObject * self, PyObject * args,
...

		{
			...
			n = PyTuple_Size(args);
			TRY(newargs =
			    PyTuple_GetSlice(args, 1, n), (PyObject *) NULL);
		...
		if (isARRAY(olevels)) {
			GET_ARR(alevels, olevels, PyArray_DOUBLE, 1,
				PyObject *);
			...
			if (nlevels > 2) {
				...
				return
				    ERRSS
				    ("contour: only 1 or 2 levels allowed.");
			}
			for (i = 0; i < nlevels; i++)
				levels[i] = lev[i];
		...
		if (!
		    (retval =
		     Py_BuildValue("[O,O,O]", Py_None, Py_None, Py_None))) {
			clearArrayList();
			return
			    ERRSS
			    ("contour: unable to create return value list.");
		}
		if (ntotal == 0) {
			clearArrayList();
		...
		RET_ARR(axcp, 1, &ntotal, PyArray_DOUBLE, (char *)xcp,
			PyObject *);
		SET_OWN(axcp);
		NEW_MEM(ycp, ntotal, double, PyObject *);
		RET_ARR(aycp, 1, &ntotal, PyArray_DOUBLE, (char *)ycp,
			PyObject *);
		SET_OWN(aycp);
		...
		if (PyList_SetItem(retval, 0, anp) < 0 ||
			...
			clearMemList();
			return
			    ERRSS("contour was unable to build return list.");
		}
...
/*  -------------------------------------------------------------------- */
	...
	static char xid__doc__[] =
	    " xid( n )\n"
	    "     returns the X window id for gist window [0..7].\n"
	    "\n" "   SEE ALSO: window\n";

	static PyObject *xid(PyObject * self, PyObject * args) {
		int n;

		if (!PyArg_ParseTuple(args, "i", &n)) {
			return ERRSS("xid takes one int argument.");
		}
		if (n < 0 || n > 7) {
			return
...
/*  -------------------------------------------------------------------- */
	...
	static PyObject *keybd_focus(PyObject * self, PyObject * args) {
		...
		if (!PyArg_ParseTuple(args, "i|i", &n, &win)) {
			return
			    ERRSS
			    ("keybd_focus takes one or two int arguments.");
		}
	...
	static long build_kwt(PyObject * kd, char *kwlist[], PyObject * kwt[]) {
		...
		for (i = 0; i < n; i++) {
			...
			if (!verify_kw(kword, kwlist)) {
				sprintf(errstr, "Unrecognized keyword: %s",
					kword);
				(long)ERRSS(errstr);
				return -1;
			}
...
#define N_KEYWORDS 3
	...
	static PyObject *bytscl(PyObject * self, PyObject * args, PyObject * kd) {
		...
		TRY(zc =
		    PushColors(z, len, zmin, zmax, scale, offset),
		    (PyObject *) NULL);
...
/* Expand tildes and environment variables in pathnames. */
	static char *expand_pathname(const char *name) {
		 ...
		 */
		TRY(m =
		    (PyObject *) PyImport_AddModule("posixpath"), (char *)NULL);
		TRY(d = PyModule_GetDict(m), (char *)NULL);
		TRY(xpnduser =
		    PyDict_GetItemString(d, "expanduser"), (char *)NULL);
		TRY(xpndvars =
		    PyDict_GetItemString(d, "expandvars"), (char *)NULL);
...
/*  hcp_finish */
	...
	static PyObject *hcp_finish(PyObject * self, PyObject * args) {
		...
		if (n < -1 || n > 7) {
			return
			    ERRSS
			    ("hcp_finish argument must be -1 through 7 inclusive");
		}
		p = PyString_FromString(GetHCPname(n));

		if (n >= 0)
			engine =
			    ghDevices[n].hcp ? ghDevices[n].hcp : hcpDefault;
		else
			engine = hcpDefault;
		if (engine) {
...
#define N_KEYWORDS 3
	...
	static PyObject *limits(PyObject * self, PyObject * args, PyObject * kd) {
		...
		if (!PyArg_ParseTuple(args, "|OOOO",
			...
			return
			    ERRSS
			    ("limits takes at most 4 non-keyword arguments.");
		}
		...
		if (xmin_ob && PyTuple_Check(xmin_ob)) {	/* Restore saved limits. */
			...
			if (!unpack_limit_tuple
			    (xmin_ob, new_limits, &new_flags)) {
		...
		if (xmin_ob) {
			...
			if (0 == j) {	/* Error */
				return
				    ERRSS
				    ("bad xmin argument: Use float or 'e'");
		...
		if (xmax_ob) {
			...
			if (0 == j) {	/* Error */
				return
				    ERRSS
				    ("bad xmax argument: Use float or 'e'");
		...
		if (ymin_ob) {
			...
			if (0 == j) {	/* Error */
				return
				    ERRSS
				    ("bad ymin argument: Use float or 'e'");
		...
		if (ymax_ob) {
			...
			if (0 == j) {	/* Error */
				return
				    ERRSS
				    ("bad ymax argument: Use float or 'e'");
...
/*  mesh_loc */
	...
	static PyObject *mesh_loc(PyObject * self, PyObject * args) {
		...
		if (PyTuple_Size(args) < 2) {
			return
			    ERRSS("mesh_loc requires at least two arguments");
		}
		...
		if (isARRAY(y0op)) {
			TRY(addToArrayList
			    ((PyObject *) (y0ap = (PyArrayObject *)
					   PyArray_ContiguousFromObject(y0op,
									PyArray_DOUBLE,
									1, 0))),
			    (PyObject *) PyErr_NoMemory());
			n = A_SIZE(y0ap);
			TRY(addToArrayList
			    ((PyObject *) (x0ap = (PyArrayObject *)
					   PyArray_ContiguousFromObject(x0op,
									PyArray_DOUBLE,
									1, 0))),
			    (PyObject *) PyErr_NoMemory());
			if (n != A_SIZE(x0ap)) {
			...
			return
			    ERRSS
			    ("(y0, x0) must be floating point scalars or arrays.");
		}
...
#define MOUSE_CLICK
		...
		if (n < 0 || !ghDevices[n].display) {
			return
			    ERRSS
			    ("no current graphics window for mouse function");
		}
...
#else
		return
		    ERRSS("no mouse function in this version of Python/Gist");
#endif
	}
...
#define N_KEYWORDS 2
	...
	static PyObject *palette(PyObject * self, PyObject * args,
		...
		case 4:	/* (red, green, blue, gray) given */
			TRY(grayop =
			    PyTuple_GetItem(args, 3), (PyObject *) NULL);
		...
		case 1:	/* (filename) or (source_window_number) given */
			if (query) {
				return
				    ERRSS
				    ("query requires (r,g,b) arrays as arguments");
			}
		...
		if (query) {	/* Retrieve the current palette. */
			...
			if (red) {
				...
				if (nColors <= 0) {
					char s[1024];
					sprintf(s, "%s: Gist palette not found",
						filename);
					clearArrayList();
					return ERRSS(s);
				}
...
#define N_KEYWORDS 15
	...
	static PyObject *plc(PyObject * self, PyObject * args, PyObject * kd) {
		...
		if (A_DIM(pyMsh.y, 0) != jMax || A_DIM(pyMsh.y, 1) != iMax) {
			...
			return
			    ERRSS
			    ("Z array must match (y, x) mesh arrays in shape");
		}
		z = (double *)A_DATA(zap);
		get_mesh(&mesh);
		if (mesh.iMax != iMax || mesh.jMax != jMax) {
			return
			    ERRSS
			    ("z array must have same dimensions as mesh in plc");
		}

		/* set legend and hide in gistD */
		TRYS(CheckDefaultWindow())
		    if (!LegendAndHide
			("\001: plc, ", z_name, y_name, x_name, r_name, kwt,
			 plcKeys))
		...
		if (!levels) {
			...
			GetPCrange(&zmin, &zmax, z, mesh.reg, gistD.region,
				   iMax, jMax);
...
#define N_KEYWORDS 30
	...
	static PyObject *pldefault(PyObject * self, PyObject * args,
		...
		if (kwt[15]) {
			SETKW(kwt[15], gistA.t.height, setkw_double,
			      dfltKeys[15]);
...
#define N_KEYWORDS 5
	...
	static PyObject *pldj(PyObject * self, PyObject * args, PyObject * kd) {
		...
		char *errstr =
		    "pldj requires exactly four non-keyword arguments";

		SETJMP0;

		if (!PyArg_ParseTuple
		    (args, "OOOO", &op[0], &op[1], &op[2], &op[3])) {
			return ERRSS(errstr);
		}

		for (i = 0; i < 4; i++)
			TRY(addToArrayList
			    ((PyObject *) (ap[i] = (PyArrayObject *)
					   PyArray_ContiguousFromObject(op[i],
									PyArray_DOUBLE,
									1, 0))),
			    (PyObject *) PyErr_NoMemory());
		...
		for (i = 1; i < 4; i++)
			if (A_SIZE(ap[i]) != n) {
				...
				return
				    ERRSS
				    ("pldj arguments must all be the same size");
			}
		...
		if (!LegendAndHide
		    ("pldj, ", x0_name, y0_name, x1_name, y1_name, kwt,
		     pldjKeys))
...
#define N_KEYWORDS 36
	...
	static PyObject *pledit(PyObject * self, PyObject * args, PyObject * kd) {
		...
		default:
			return
			    ERRSS
			    ("pledit function takes no more than two arguments");
		}
		...
		if (n_element < 0) {
			if (curElement >= 0) {
				...
				return
				    ERRSS
				    ("no current graphical element for pledit");
			}
		}
		if (n_element >= 0 || n_contour >= 0) {
			...
			if (type == 0) {
				return
				    ERRSS
				    ("no such graphical element for pledit");
			}
		}
		...
		if (kwt[2]) {
			...
			gistA.m.color = gistA.f.color = gistA.t.color =
			    gistA.l.color;
		}
		...
		if (kwt[19]) {
			SETKW(kwt[19], gistA.t.height, setkw_double,
			      editKeys[19]);
			gistA.t.height *= ONE_POINT;
		}
		...
		if (kwt[25]) {	/* region */
			...
			SETKW(kwt[25], gistD.region, setkw_integer,
			      editKeys[25]);
		}
		if (kwt[26]) {	/* boundary */
			if (type != 4) {
				return
				    ERRSS
				    ("boundary = in pledit allowed only for plm");
			}
			SETKW(kwt[26], gistD.boundary, setkw_boolean,
			      editKeys[26]);
		}
		...
		if (kwt[27]) {	/* levs */
			...
			if (type != 7) {
				return
				    ERRSS
				    ("levs = in pledit allowed only for plc");
			}
		...
		if (kwt[28]) {	/* scale */
			if (type != 6) {
				return
				    ERRSS
				    ("scale = in pledit allowed only for plv");
			}
		...
		if (kwt[29]) {	/* scalem */
			...
			if (type != 6) {
				return
				    ERRSS
				    ("scalem = in pledit allowed only for plv");
			}
		...
		if (kwt[30]) {	/* dx */
			...
			if (type != 3) {
				return
				    ERRSS
				    ("dx = in pledit allowed only for plt");
			}
		...
		if (kwt[31]) {	/* dy */
			...
			if (type != 3) {
				return
				    ERRSS
				    ("dy = in pledit allowed only for plt");
			}
		...
		if (kwt[35]) {	/* inhibit */
			if (type != 4) {
				return
				    ERRSS
				    ("inhibit = in pledit allowed only for plm");
			}
			SETKW(kwt[35], gistD.inhibit, setkw_integer,
			      editKeys[35]);
		}
		if (legend) {
...
#define N_KEYWORDS 9
	...
	static PyObject *plf(PyObject * self, PyObject * args, PyObject * kd) {
		...
		char *errstr =
		    "plf requires 2D arguments (z [ , y, x, ireg ] )";
		...
		if (isARRAY(zop) && (A_TYPE(zop) == PyArray_UBYTE)) {
			...
			if (isARRAY(zop)
				...
				GET_ARR(zap, zop, PyArray_DOUBLE, 2,
					PyObject *);
				z = (double *)A_DATA(zap);
			} else {
				z = 0;
		...
		PyFPE_START_PROTECT("plf", return 0)
		    curElement =
		    GdFillMesh(NOCOPY_MESH, &mesh, gistD.region, zc, iMax);
		PyFPE_END_PROTECT(dummy)
		    clearArrayList();
		/* 
		 *  04/22/03 LLC 
...
#define N_KEYWORDS 5
	...
	static PyObject *pli(PyObject * self, PyObject * args, PyObject * kd) {
		...
		char *errstr =
		    "pli requires arguments (z [ , [ x0, y0, ] x1, y1 ] )";
		...
		if (!z && !zc)
			return
			    ERRSS
			    ("pli needs at least one non-keyword argument");
		...
		TRYS(CheckDefaultWindow())
		    CheckDefaultPalette();
		if (!LegendAndHide
		    ("pli, ", z_name, (char *)0, (char *)0, (char *)0, kwt,
		     pliKeys))
...
#define N_KEYWORDS 8
	...
	static PyObject *plm(PyObject * self, PyObject * args, PyObject * kd) {
		...
		char *errstr =
		    "plm takes 1-3 non-keyword arguments: (y, x, ireg).";
		...
		TRYS(CheckDefaultWindow())
		    if (!LegendAndHide
			("plm, ", y_name, x_name, r_name, (char *)0, kwt,
			 plmKeys))
		...
		if (!pyMsh.y) {
			return
			    ERRSS
			    ("no current mesh - use plmesh(y, x) to initialize");
		}
...
#define N_KEYWORDS 1
	...
	static PyObject *plmesh(PyObject * self, PyObject * args, PyObject * kd) {
		...
		char *errstr =
		    "plmesh takes 0-3 non-keyword arguments: (y, x, ireg).";
...
/*  -------------------------------------------------------------------- */
	...
	static char plq__doc__[] =
	    ...
	    "or legend_list = plq() **** returns number of elements ****\n"
	...
	static PyObject *plq(PyObject * self, PyObject * args) {
		...
		default:
			return
			    ERRSS
			    ("plq function takes no more than two arguments");
		}
		...
		if (n_element >= 0) {
			...
			for (i = 0; (type = GdSetElement(i)) != E_NONE; i++) {
				...
				if ((type == E_LINES || type == E_CONTOURS)
					...
					else if (gistA.m.type >= 1
						 && gistA.m.type <= 5)
						marker[0] =
						    specialMarkers[gistA.m.
								   type - 1];
					else
						marker[0] = '?';
					PrintFunc(marker);
...
/*  -------------------------------------------------------------------- */
	...
	static PyObject *plremove(PyObject * self, PyObject * args) {
		...
		default:
			return
			    ERRSS
			    ("plremove function takes no more than one argument");
		}
		...
		if (n_element < 0) {
			if (curElement >= 0) {
				...
				return
				    ERRSS
				    ("no current graphical element for plremove");
			}
		}
		if (n_element >= 0) {
			/* retrieve specified element */
...
/*  -------------------------------------------------------------------- */
	...
	static PyObject *plsys(PyObject * self, PyObject * args) {
		int n = -9999, n0;
		char *errstr =
		    "Error: plsys takes zero or one integer argument.";
...
#define N_KEYWORDS 10
	...
	static PyObject *plt(PyObject * self, PyObject * args, PyObject * kd) {
		...
		if (!PyArg_ParseTuple(args, "sdd", &text, &x, &y)) {
			return
			    ERRSS
			    ("plt requires exactly three non-keyword arguments");
		}
...
#define N_KEYWORDS 8
	...
	static PyObject *plv(PyObject * self, PyObject * args, PyObject * kd) {
		...
		TRYS(CheckDefaultWindow())
		    if (!LegendAndHide
			("plv, ", v_name, u_name, y_name, x_name, kwt, plvKeys))
		...
		if (kwt[7]) {	/* scale= keyword */
			...
			GetPCrange(&ymin, &ymax, mesh.y, mesh.reg, gistD.region,
				   iMax, jMax);
			GetPCrange(&umin, &umax, u, mesh.reg, gistD.region,
				   iMax, jMax);
			GetPCrange(&vmin, &vmax, v, mesh.reg, gistD.region,
				   iMax, jMax);
...
/* Create a default region array for the current mesh. */
	static int set_def_reg(int nr, int nc) {
		...
		newlen = ne + nc + 1;
		TRY(ra1 =
		    (PyArrayObject *) PyArray_SimpleNew(1, &newlen,
							PyArray_LONG), 0);
		p1 = (int *)A_DATA(ra1);
...
/* Create a non-default region (mesh) array from the passed-in object. */
	static long set_reg(PyObject * op) {
		...
		if (!pyMsh.y) {
			return (long)
			    ERRSS
			    ("No current mesh - ireg not set - set (y, x) first");
		}
		nr = A_DIM(op, 0);
		nc = A_DIM(op, 1);
...
/* Create a triangulation (mesh) array. */
	static long set_tri(PyObject * top) {
		...
		if (A_DIM(pyMsh.triangle, 0) != nr
		    || A_DIM(pyMsh.triangle, 1) != nc) {
	...
	static long setkw_boolean(PyObject * v, int *t, char *kw) {
		char buf[256];
		char *format =
		    "%s keyword requires argument of 0 (False) or 1 (True)";
	...
	static long unpack_color_tuple(PyObject * ob,
		...
		for (i = 0; i < 3; i++) {
			...
			if (PyInt_Check(item)) {
				...
				return (long)
				    ERRSS
				    ("Color tuple: expected integer value ");
			}
		}
		return 1;
	}
...
/* Used only by limits() */
	static long unpack_limit_tuple(PyObject * ob, double limits[],
		...
		if (5 != size) {
			return (long)
			    ERRSS
			    ("Old limits must have four doubles + 1 integer");
		}
		for (i = 0; i < 4; i++) {
			if ((item = PyTuple_GetItem(ob, i)) == 0) {
				return (long)
   ...
   [nverts, xyzverts, values, nvertb, xyzvertb, valueb]            */
	{
		PyObject *oplane,
		    *onverts,
		    *oxyzverts, *ovalues = (PyObject *) NULL, *oreturn_value,
		    /* return values */
		    *ornverts, *orxyzverts, *orvalues = (PyObject *) NULL,
		    /* additional slice2x return values */
		    *ornvertb, *orxyzvertb, *orvalueb = (PyObject *) NULL;
		PyArrayObject *aplane = (PyArrayObject *) NULL,
		    *anverts, *axyzverts, *avalues = (PyArrayObject *) NULL;
		ArrayObject *rnverts = (ArrayObject *) NULL,
		    ...
		    *nkeep2,
		    *mask2,
		    *list = (ArrayObject *) NULL, *listc = (ArrayObject *) NULL;
		double *planed = 0,
		    *xyzvertsd = 0,
		    *valuesd = (double *)NULL,
		    *rxyzvertsd = 0,
		    ...
		    *xyzcd = 0,
		    *xyzc0d = 0,
		    *valuecd = (double *)NULL, *valuec0d = (double *)NULL;
		int *nvertsd = 0,
		    *rnvertsd = 0,
		    *rnvertbd = 0,
		    *nvertcd = 0,
		    ...
		    *nkeepd = 0,
		    *nkeep2d = 0,
		    *prevd = 0, *nextd = 0, *lastd = 0, *listd = 0, *listcd = 0;
		Uchar *keepd = 0,
		    *mask2d = 0,
		    *valuesc = (Uchar *) NULL,
		    *rvaluecc = (Uchar *) NULL,
		    *rvaluesc = (Uchar *) NULL,
		    *rvaluebc = (Uchar *) NULL,
		    *valuecc = (Uchar *) NULL, *valuec0c = (Uchar *) NULL;
		char atype = '\0';
		int _slice2x = 0,
		    plane_is_scalar,
		    isum,
		    ...
		    listc_length = 0,
		    listc_length1 = 0,
		    listc0_length = 0, node, sumt, sumv, xdims[2];
		double dplane = 0.;

		if (!PyArg_ParseTuple
		    (args, "OOO|Oi", &oplane, &onverts, &oxyzverts, &ovalues,
		     &_slice2x)) {
		...
		if (isARRAY(ovalues)) {
			if (A_TYPE(ovalues) == PyArray_DOUBLE
				...
				GET_ARR(avalues, ovalues, Py_GpColor, 1,
					PyObject *);
				...
				return
				    ERRSS
				    ("Data type for values must be 'b' or 'd'.");
			}
		}
		...
		if (!plane_is_scalar) {
			...
			return
			    ERRSS
			    ("Not sure what kind of slice you're asking for.");
		}
		...
		TRY(nkeep =
		    allocateArray(A_SIZE(anverts), 'i', 0), PyErr_NoMemory());
		...
		for (i = 0; i < nkeep->size; i++) {
			for (j = 0; j < nvertsd[i]; j++) {
				nkeepd[i] +=
				    (int)(dpd[k] >=
					  PyFloat_AsDouble(_slice2_precision));
				k++;
			}
		}
		for (i = 0; i < nkeep->size; i++) {
		...
		if (list1_length != 0) {
			...
			TRY(nvertc =
			    allocateArray(list1_length, 'i', 0),
			    PyErr_NoMemory());
			nvertcd = (int *)(nvertc->data);
			TRY(list =
			    allocateArray(list_length, 'i', 0),
			    PyErr_NoMemory());
			listd = (int *)(list->data);
			TRY(xyzc =
			    allocateArray(list_length * 3, 'd', 0),
			    PyErr_NoMemory());
			xyzcd = (double *)(xyzc->data);
			if (avalues && node == 0) {
				TRY(valuec =
			...
			for (i = 0, k = 0, sumv = 0, sumt = 0; i < nkeep->size;
			     i++) {
				if (nkeepd[i] != 0 && nkeepd[i] != nvertsd[i]) {
					nvertcd[k] = nvertsd[i];
					if (avalues && node == 0) {
		...
		if (_slice2x) {
			...
			if (!PyFloat_AsDouble(_slice2_precision))
				if (list1_length == 0) {
						...
						for (j = 0; j < nvertsd[i]; j++) {
							...
							if (avalues
								...
								if (atype ==
								    'd')
									rvaluebd
									    [(sumv + j)] = valuesd[(sumt + j)];
								else
									rvaluebc
									    [(sumv + j)] = valuesc[(sumt + j)];
							}
						}
						k++;
						sumv += nvertsd[i];
...
/* freeArray (nkeep, 0); Need nkeep and nverts instead of mask0 */
		...
		for (i = 0; i < nkeep->size; i++) {
			...
			listc0_length +=
			    (nkeepd[i] == nvertsd[i]) ? nvertsd[i] : 0;
		}
		...
		RET_ARR(ornverts, 1, &(rnverts->size), PyArray_LONG,
		...
		if (rxyzvertb == NULL) {
			...
			/* Build the rest */
			RET_ARR(ornvertb, 1, &(rnvertb->size), PyArray_LONG,
	...
	static PyObject *window(PyObject * self, PyObject * args, PyObject * kd) {
		...
		if (PyTuple_Size(args) == 1) {	/* Window() was called with an argument. */
			if (n < 0 || n > 7) {
				return
				    ERRSS
				    ("graphics windows are numbered from 0 to 7");
			}
		...
		if (nGiven || kwt[0] || kwt[1] || kwt[2]) {
			...
			if (kwt[2]) {
				...
				SETKW(kwt[2], privmap, setkw_boolean,
				      windowKeys[2]);
			}
			if (kwt[10]) {
				...
				SETKW(kwt[10], rgb, setkw_boolean,
				      windowKeys[10]);
			}
			if (kwt[11]) {
				extern unsigned long gx_parent;
				/* parent= keyword */
				SETKW(kwt[11], parent, setkw_integer,
				      windowKeys[11]);
				gx_parent = parent;
			}
			if (kwt[12]) {
				extern int gx_xloc;
				/* xpos= keyword */
				SETKW(kwt[12], xpos, setkw_integer,
				      windowKeys[12]);
				gx_xloc = xpos;
			}
			if (kwt[13]) {
				extern int gx_yloc;
				/* ypos= keyword */
				SETKW(kwt[13], ypos, setkw_integer,
				      windowKeys[13]);
				gx_yloc = ypos;
			}
			...
			if (nGiven ? (!display || display[0])
				...
				gist_rgb_hint = rgb;
				engine =
				    GpFXEngine(windowNames[n], 0, dpi, display);
				if (!engine) {
					SAFE_FREE(display);
					return
					    ERRSS
		...
		if (kwt[3]) {
			...
			if (hcp && hcp[0]) {
				...
				if (len > 3
				    && strcmp(&hcp[len - 3], ".ps") == 0) {
					engine =
					    GpPSEngine(windowNames[n], 0,
						       hcpDump, SetHCPname(n,
									   hcp,
									   1));
					if (!engine) {
						return
		...
		if (!ghDevices[n].display && !ghDevices[n].hcp) {
			...
			if (kwt[6]) {
				...
				char *style = 0;
				SETKW(kwt[6], style, setkw_string,
				      windowKeys[6]);
				drawing = ghDevices[n].drawing;
				if (drawing) {
					ghDevices[n].drawing = 0;
			...
			if (kwt[7] && wait_for_expose) {
				int wait;
				SETKW(kwt[7], wait, setkw_boolean,
				      windowKeys[7]);
...
/*  window_geometry */
	...
	static char window_geometry__doc__[] =
	...
	static PyObject *window_geometry(PyObject * self, PyObject * args) {
		int win = -1;
		Engine *engine;
		GpXYMap *map;
		...
		if (!PyArg_ParseTuple(args, "|i", &win)) {
			return
			    ERRSS
			    ("window_geometry takes zero or one argument.");
		}
...
#define MARGIN(SIDE) (((XEngine *)engine)->SIDE##Margin)
			xbias =
			    (MARGIN(left) - map->x.offset + 0.5) / map->x.scale;
			ybias =
			    (MARGIN(top) - map->y.offset + 0.5) / map->y.scale;
#undef MARGIN
...
/*  window_exists */
	...
	static char window_exists__doc__[] =
	    "DOCUMENT window_exists(n)\n"
	    ...
	    "      window_select, window_list, window_geometry\n" "";

	static PyObject *window_exists(PyObject * self, PyObject * args) {
		int n;

		if (!PyArg_ParseTuple(args, "i", &n)) {
			return ERRSS("window_exists takes one argument.");
		}
...
/*  window_list */
	...
	static char window_list__doc__[] =
	    "DOCUMENT window_list()\n"
	    "     The function window_list returns the list of existing windows as a\n"
	    "     vector of longs or nil if no window currently exists.\n"
	    "SEE ALSO: window, current_window, redraw, fma, limits, window_geometry\n"
	    "      window_select, window_exists, window_geometry\n" "";

	static PyObject *window_list(PyObject * self, PyObject * args) {
		if (!PyArg_ParseTuple(args, "")) {
			return ERRSS("window_list takes no argument.");
		}
		long i, n;
...
/*  window_select */
	...
	static char window_select__doc__[] =
	    ...
	    "SEE ALSO: window, current_window, redraw, fma, limits, window_geometry\n"
	    "      window_exists, window_list, window_geometry\n" "";

	static PyObject *window_select(PyObject * self, PyObject * args) {
		int n;

		if (!PyArg_ParseTuple(args, "i", &n)) {
			return ERRSS("window_select takes one argument.");
		}
...
/*  -------------------------------------------------------------------- */
	...
	static PyObject *zoom_factor(PyObject * self, PyObject * args) {
		if (!PyArg_ParseTuple(args, "d", &gxZoomFactor)) {
			return
			    ERRSS
			    ("Zoomfactor takes one floating point argument.");
		}
		if (gxZoomFactor < 0.0)
...
/* wire up pyg_on_connect function */
	static PyObject *pyg_register(PyObject * self, PyObject * args) {
		...
		if (!PyArg_ParseTuple(args, "|OO", &func1, &func2) ||
			...
			return
			    ERRSS("pyg_register takes two function arguments.");
		}
		if (pyg_connector) {
			Py_DECREF(pyg_connector);
		}
		pyg_connector = func1;
		Py_INCREF(func1);
		if (pyg_keyhandler) {
			Py_DECREF(pyg_keyhandler);
		}
 ...
 */
	...
	static void u_inputfd_ready(void *c) {
		...
		if (pyg_inputhook_func && pyg_inputhook_FILE) {
			...
			res =
			    PyObject_CallFunctionObjArgs(pyg_inputhook_func,
							 arg, NULL);
 ...
 * Michiel de Hoon 2003.04.06. */
	...
	static PyObject *get_legend(GeLegendBox * legends) {
		PyObject *dictionary;
		PyObject *textStyle =
		    get_text_attributes(&(legends->textStyle));
	...
	static PyObject *get_tick_style(GaTickStyle * ticks) {
		PyObject *frameStyle =
		    get_line_attributes(&(ticks->frameStyle));
...
/*----------------------------------------------------------------------------*/
	...
	int set_line_attributes(PyObject * dictionary,
				GpLineAttribs * attributes) {
		PyObject *width;
		PyObject *color;
		PyObject *type;
		char *ctype;
  ...
  /*--------------------------------------------------------------------------*/
		if (PyInt_Check(color)) {
			...
			if (lcolor < 0) {
				ERRMSG
				    ("number for color should be non-negative");
				return 0;
			}
	...
	int set_text_attributes(PyObject * dictionary,
				GpTextAttribs * attributes) {
		PyObject *font;
		PyObject *height;
		PyObject *orient;
		PyObject *alignH;
  ...
  /*--------------------------------------------------------------------------*/
		...
		if (!strcmp(sorient, "right"))
			...
			attributes->orient = TX_DOWN;
			 ...
			 && attributes->orient < 0) { /* ok, xft angle */ ;
		} else {
			ERRMSG
  ...
  /*--------------------------------------------------------------------------*/
		tickLen =
		    (PyArrayObject *) PyDict_GetItemString(dictionary,
							   "tickLen");
		if (!tickLen) {
  ...
  /*--------------------------------------------------------------------------*/
	...
	static PyObject *set_style(PyObject * self, PyObject * args) {
		...
		int ilandscape;
		int ok =
		    PyArg_ParseTuple(args, "O!", &PyDict_Type, &dictionary);
		if (!ok)
			return NULL;
		...
		contourlegend =
		    PyDict_GetItemString(dictionary, "contourlegend");
		if (!contourlegend)
			return
			    ERRSS("key contourlegend not found in dictionary");
   ...
   external linkage. */
    ...
    PyModule_AddStringConstant(m, "version", PYGIST_VERSION);
		...
		if (0 != Py_AtExit(CleanUpGraphics)) {
			TO_STDERR
			    ("Gist: Warning: Exit procedure not registered\n");
			flush_stderr();
		}
