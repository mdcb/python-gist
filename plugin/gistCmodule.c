...
static PyObject *plc(PyObject * self, PyObject * args, PyObject * kd)
{
	...
	PyFPE_START_PROTECT("plc", return 0)
	    ...
	    GdContours(NOCOPY_MESH, &mesh, gistD.region, z, levels,
		       (int)nLevels);
	PyFPE_END_PROTECT(dummy) Py_DECREF(zap);
	SAFE_FREE(levels);
	array_list_length = 0;
	mem_list_length = 0;
	if (curElement < 0) {
...
static PyObject *pldj(PyObject * self, PyObject * args, PyObject * kd)
{
	...
	PyFPE_END_PROTECT(dummy) clearArrayList();
	if (curElement < 0) {
...
static PyObject *pledit(PyObject * self, PyObject * args, PyObject * kd)
{
	...
	if (kwt[29]) {		/* scalem */
		...
		PyFPE_START_PROTECT("pledit", return 0) gistD.scale *= scalem;
	...
	if (kwt[30]) {		/* dx */
		...
		PyFPE_START_PROTECT("pledit", return 0) gistD.x0 += x0;
	PyFPE_END_PROTECT(dummy)}
	if (kwt[31]) {		/* dy */
		double y0;
		if (type != 3) {
		...
		PyFPE_START_PROTECT("pledit", return 0) gistD.y0 += y0;
	PyFPE_END_PROTECT(dummy)}
	if (kwt[32]) {
		int edgetype = 0;
...
static PyObject *plg(PyObject * self, PyObject * args, PyObject * kd)
{
	...
	curElement = -1;
	PyFPE_START_PROTECT("plg", return 0) curElement = GdLines(length, x, y);
	PyFPE_END_PROTECT(dummy) clearArrayList();
	clearMemList();
...
static PyObject *pli(PyObject * self, PyObject * args, PyObject * kd)
{
	...
	PyFPE_END_PROTECT(dummy) removeFromArrayList((PyObject *) zap);
	if (convertedZ)
		free(zc);
	if (curElement < 0) {
...
static PyObject *plm(PyObject * self, PyObject * args, PyObject * kd)
{
	...
	PyFPE_START_PROTECT("plm", return 0)
	    ...
	    GdMesh(NOCOPY_MESH, &mesh, gistD.region, gistD.boundary,
		   gistD.inhibit);
	PyFPE_END_PROTECT(dummy) if (curElement < 0) {
...
{
	PyObject *oplane,
	    *onverts, *oxyzverts, *ovalues = (PyObject *) NULL, *oreturn_value,
	    /* return values */
	...
	ArrayObject *rnverts = (ArrayObject *) NULL,
	    ...
	    *nvertc = (ArrayObject *) NULL,
	    *xyzc = (ArrayObject *) NULL, *valuec = (ArrayObject *) NULL,
	    ...
	    *nkeep,
	    *nkeep2,
	    *mask2, *list = (ArrayObject *) NULL, *listc = (ArrayObject *) NULL;
	double *planed = 0, *xyzvertsd = 0, *valuesd =
	    (double *)NULL, *rxyzvertsd = 0, *rvaluecd =
	    (double *)NULL, *rvaluesd = (double *)NULL, *rxyzvertbd =
	    0, *rvaluebd = (double *)NULL, *dpd =
	    (double *)NULL, *ndpd = 0, *xyzcd = 0, *xyzc0d =
	    0, *valuecd = (double *)NULL, *valuec0d = (double *)NULL;
	int *nvertsd = 0, *rnvertsd = 0, *rnvertbd = 0, *nvertcd =
	    0, *nvertc0d = 0, *nkeepd = 0, *nkeep2d = 0, *prevd =
	    0, *nextd = 0, *lastd = 0, *listd = 0, *listcd = 0;
	Uchar *keepd = 0, *mask2d = 0, *valuesc =
	    (Uchar *) NULL, *rvaluecc = (Uchar *) NULL, *rvaluesc =
	    (Uchar *) NULL, *rvaluebc = (Uchar *) NULL, *valuecc =
	    (Uchar *) NULL, *valuec0c = (Uchar *) NULL;
	char atype = '\0';
	int _slice2x = 0,
	    plane_is_scalar,
	    isum,
	    ...
	    list2_length = 0,
	    listc_length = 0,
	    listc_length1 = 0, listc0_length = 0, node, sumt, sumv, xdims[2];
	double dplane = 0.;
...
/* freeArray (nkeep, 0); Need nkeep and nverts instead of mask0 */
	...
	if (list1_length == 0 && listc_length == 0) {
		if (rnverts) {
			RET_ARR(ornverts, 1, &(rnverts->size),
				PyArray_LONG, (char *)rnvertsd, PyObject *);
			SET_OWN(ornverts);
		} else {
			Py_INCREF(Py_None);
			ornverts = Py_None;
		...
		if (rxyzvertb == NULL) {
			...
			RET_ARR(ornvertb, 1, &(rnvertb->size),
				PyArray_LONG, (char *)rnvertbd, PyObject *);
			SET_OWN(ornvertb);
	...
	if (list1_length != 0) {
		...
		for (i = 0; i < list_length; i++)
			ndpd[i] =
			    dpd[listd[i]] - PyFloat_AsDouble(_slice2_precision);
		freeArray(dp, 0);
...
static PyObject *window_geometry(PyObject * self, PyObject * args)
{
	...
	if (!PyArg_ParseTuple(args, "|i", &win)) {
		return ERRSS("window_geometry takes zero or one argument.");
	}
