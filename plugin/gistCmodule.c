...
static PyObject *debug_array (PyObject * self, PyObject * args)
{
 ...
 TO_STDOUT(" Reference count %ld, size %ld.\n", oarray->ob_refcnt,
 ...
 TO_STDOUT("Data pointer: %p; nd %d; dim1 %ld; type %c.\n", aarray->data,
...
static PyObject *plf (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  if (isARRAY(zop) && (A_TYPE(zop) == PyArray_UBYTE)) {
    ...
    if (isARRAY(zop) && ((A_TYPE(zop) == PyArray_DOUBLE)||(A_TYPE(zop) == PyArray_FLOAT))) {
      GET_ARR (zap, zop, PyArray_DOUBLE, 2, PyObject *);
      z = (double *) A_DATA (zap);
    } else {
      z = 0;
...
static PyObject *plfp (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  } else if (isARRAY(zop) && ((A_TYPE(zop) == PyArray_DOUBLE)||(A_TYPE(zop) == PyArray_FLOAT))) {
...
static PyObject *pli (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  if (isARRAY(zop) && (A_TYPE(zop) == PyArray_UBYTE)) {
    ...
    if ( A_NDIM(zop) == 2 )  {
       ...
       /*  NXxNYx3 */
       if ( A_DIM(zop,2) != 3 )  {
          return ERRSS ("expecting NXxNY or NXxNYx3 array as argument to pli"); 
       }
       GET_ARR (zap, zop, Py_GpColor, 3, PyObject *);
       zc = (GpColor *) A_DATA (zap);
       rgb = 1;
   ...
   [nverts, xyzverts, values, nvertb, xyzvertb, valueb]            */
{
 ...
 if (isARRAY (ovalues)) {
    if (A_TYPE (ovalues) == PyArray_DOUBLE || A_TYPE (ovalues) == PyArray_FLOAT) {
...
static void  u_inputfd_ready(void *c) {
   ...
   PyGILState_STATE gilstate = PyGILState_Ensure();
         ...
         res = PyObject_CallFunctionObjArgs(	pyg_inputhook_func,arg,NULL);
...
static PyObject *pyg_inputhook (PyObject * self, PyObject * args) {
  ...
  if (!PyArg_ParseTuple(args, "iO", &fd, &func) ||
    ...
    return ERRSS ("pyg_inputhook takes a file descriptor and a function callback.");
  }
...
{ char *alignH;
  ...
  { 
    case TX_RIGHT: strcpy(orient,"right"); break;
    case TX_UP: strcpy(orient,"up"); break;
    case TX_LEFT: strcpy(orient,"left"); break;
    case TX_DOWN: strcpy(orient,"down"); break;
    default: 
      if (attributes->orient>3) strcpy(orient,"right");
      else snprintf(orient,sizeof(orient),"%d",attributes->orient);
      break;
  }
  dictionary =
...
{ PyObject *font;
  ...
  else if (( sscanf(sorient,"%d",&attributes->orient) == 1 ) && attributes->orient<0 ) { /* ok, xft angle */ ;}
  else {
    ERRMSG("orient should be 'right', 'up', 'left', 'down', or an angle");
    return 0;
  }
  /*--------------------------------------------------------------------------*/
...
{ 
  ...
  { "window_geometry",PYCF window_geometry,  1,     window_geometry__doc__ },
  ...
  { "pyg_inputunhook",PYCF   pyg_inputunhook,1,     pyg_inputunhook__doc__ },
